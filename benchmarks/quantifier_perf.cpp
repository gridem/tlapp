...
TEST(QuantifierPerf, Run) {
  ...
  auto existsAssign = exists(smallValues, [&] lam_arg(i) { return x == i; });

  Context ctx;
  vec.getRef(ctx) = values;

  expectBenchChecksum(
      "quant_forall_late_fail_4096", 2000, 0, [&] { return forallLateFail(ctx); });

  expectBenchPerIteration(
      "quant_exists_early_hit_4096", 5000, 1, [&] { return existsEarlyHit(ctx); });

  expectBenchPerIteration(
      "quant_exists_late_hit_4096", 2000, 1, [&] { return existsLateHit(ctx); });

  expectBenchPerIteration(
      "quant_exists_assign_16", 5000, 16, [&] { return existsAssign(ctx); });
}
